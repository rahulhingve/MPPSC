...
- **All images must load successfully** — verify every image URL actually returns a valid image (HTTP 200 with image content-type) before including it. Do not use Wikimedia thumbnail URLs that require specific size parameters listed on https://w.wiki/GHai; instead use direct full-size file URLs (e.g., `upload.wikimedia.org/wikipedia/commons/hash/hash/Filename.ext`) or the `Special:Redirect/file/Filename?width=N` endpoint which handles sizing automatically.
